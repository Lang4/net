...
public:
	...
	{
		if (offset >= binary.size()) return false;
...
public:
	...
	Stream(){}
	Stream(BYTE_ARRAY *out)
	{
		setTag(IN);
		fromBinary(out);
	}
	Stream(unsigned char *content,unsigned int len)
	{
		setTag(IN);
		BYTE_ARRAY contents;
		contents.resize(len);
	...
	template<typename type>
	Stream(type& o,unsigned char *content,unsigned int len)
	{
		setTag(IN);
		BYTE_ARRAY contents;
		contents.resize(len);
	...
	bool read(Stream &a,unsigned short tag)
	{
		unsigned char blockid = (tag & 0x0ff0) >> 4;
		if (blockid >= blocks.size())
		{
	...
	bool write(Stream &a,unsigned short tag)
	{
		unsigned char blockid = (tag & 0x0ff0) >> 4;
		if (blockid >= blocks.size())
		{
...
void operator & (Stream &ss,__class__ &o)\

#define BD(field,version)\
	if (ss.isTag(OUT))\
	{\
...
void operator & (Stream &ss,std::vector<__type__>& o)
{
	if (ss.isTag(OUT))
	{
		size_t size = o.size();
...
template<typename __type__>
void operator & (Stream &ss,std::list<__type__>& o)
{
	if (ss.isTag(OUT))
	{
		size_t size = o.size();
		ss.write(size,0);
		int index = 1;
		for (typename std::list<__type__>::iterator iter = o.beign(); iter != o.end();++iter)
		{
			ss.write(*iter,index++);
		}
	}
...
template<typename __type__>
void operator & (Stream &ss,std::set<__type__>& o)
{
	if (ss.isTag(OUT))
	{
		size_t size = o.size();
		ss.write(size,0);
		int index = 1;
		for (typename std::set<__type__>::iterator iter = o.beign(); iter != o.end();++iter)
		{
			ss.write(*iter,index++);
		}
	}
...
void operator & (Stream &ss,std::map<__key__,__value__>& o)
{
	if (ss.isTag(OUT))
	{
		size_t size = o.size();
		ss.write(size,0);
		int index = 1;
		for (typename std::map<__key__,__value__>::iterator iter = o.beign(); iter != o.end();++iter)
		{
			ss.write(iter->first,index++);
